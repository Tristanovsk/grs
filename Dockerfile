...
RUN sed -i 's#/srv/conda/envs/env_snap/snap//.snap/system#//tmp/test/.snap/system/#g' /srv/conda/envs/env_snap/snap/etc/snap.conf
RUN sed -i 's#/srv/conda/envs/env_snap/snap/.snap#//tmp/test/.snap/#g' /srv/conda/envs/env_snap/snap//etc/snap.properties
...
RUN --mount=type=secret,id=proxy_http_cnes \ 
    export http_proxy=$(cat /run/secrets/proxy_http_cnes) && export https_proxy=$(cat /run/secrets/proxy_http_cnes) && \
    snap --nosplash --nogui --modules --update-all | while read -r line; do echo "$line"; [ "$line" = "update=0" ] && sleep 2 && pkill -TERM -f "snap/jre/bin/java; done;
