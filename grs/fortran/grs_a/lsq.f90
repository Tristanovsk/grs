...
SUBROUTINE fit_poly(x,y,wt,n,m,beta,R2)
...
real(dp),intent(in) :: x(n), y(n),wt(n)
real(dp),intent(out) :: beta(0:m),R2

REAL (dp)           ::  xrow(0:m), &
                       var, covmat(231), sterr(0:m), totalSS
INTEGER             :: i, ier, iostatus, j
LOGICAL             :: fit_const = .TRUE., lindep(0:m), xfirst
...
IF (ier /= 0) THEN
  DO i = 0, m
    IF (lindep(i)) then 
     WRITE(*, '(a, 2i3)') ' Singularity detected for power: ', i, n
     R2=0.
     return
    ENDIF
