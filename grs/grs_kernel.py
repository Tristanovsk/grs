...
class Kernel():
    ...
    def set_gas_transmittance(self):
        ...
        self.Tg_diff_raster = gas_trans.get_gaseous_transmittance(gases=gases, background=False).transpose("wl", "y", "x")
    ...
    def aerosol_visible(self,
        ...
        for _aot_ref in aot_refs:
            ...
            for iy in range(0, _height, chunk):
                ...
                for ix in range(0, _width, chunk):
                    ...
                    _azi = (180. - _raa) % 360
                    _air_mass_ = Misc.air_mass(_sza, _vza)  # air_mass[:, iy:yc,ix:xc] #air_mass(_sza,_vza).values #_p_slope = prod.raster.p_slope[:, iy:yc,ix:xc]
        ...
        self.recalib_aot_ref = aot_ref_max / cams_aot_ref_mean_
